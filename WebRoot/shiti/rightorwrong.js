...
function add() {
	...
	$("input[name='rightorwrong.sctupdate']").eq(0).val(mytime);
...
function editShiTi(id) {
	$.ajax({
				...
				dataType : "json",
				success : function(data) {
					    $("input[name='rightorwrong.shiti_code']").eq(1).val(data.shiti_code);
					    $("input[name='rightorwrong.neirong']").eq(1).val(data.neirong);
					    $("input[name='rightorwrong.answer']").eq(1).val(data.answer);
					    $("input[name='rightorwrong.id']").eq(0).val(data.id);
						$("#editWin").window("open");
				}
			});		
}
...
function update() {
	var time = new Date();
	var mytime = time.toLocaleString();//获取当前时间
	$("input[name='rightorwrong.sctupdate']").eq(1).val(mytime);
	// 获取表单数据
...
function addMany() {
	...
	var time = new Date();
	var mytime = time.toLocaleString();//获取当前时间
	$("input[name='rightorwrong.sctupdate']").eq(2).val(mytime);
